...
public final class OemBadgeHelper {
	...
	interface Badger {
		...
		boolean executeBadge(int badgeCount);
	}
	...
	private class OPPOHomeBadger implements Badger {
		...
		private Object executeClassLoad(Class cls) {
			Object obj = null;
			if (!(cls == null || checkObjExists("getColorOSVERSION"))) {
				Method method = getMethod(cls, "getColorOSVERSION", null);
				if (method != null) {
	...
	public void applyCount(int badgeCount) {
		try {
			if (mBadger != null) {
				mBadger.executeBadge(badgeCount);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
