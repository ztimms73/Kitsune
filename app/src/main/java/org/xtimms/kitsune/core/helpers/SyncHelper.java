...
import org.xtimms.kitsune.core.common.SyncDevice;
...
public class SyncHelper {
    ...
    public RESTResponse syncHistory() {
        try {
            HistoryRepository repository = HistoryRepository.get(mContext);
            long lastSync = getLastHistorySync();
            JSONArray updated = repository.dumps(lastSync);
            if (updated == null) {
            ...
            if (!repository.inject(resp.getData().getJSONArray("updated"))) {
            ...
            for (int i=0;i<deleted.length(); i++) {
                ...
                repository.remove(new long[]{o.getLong("manga_id")});
            }
    ...
    public RESTResponse syncFavourites() {
        try {
            FavouritesRepository repository = FavouritesRepository.get(mContext);
