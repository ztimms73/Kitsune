...
import org.xtimms.kitsune.ui.mangalist.MangaListAdapter;
...
public class SearchResultsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements View.OnClickListener {
	...
	public SearchResultsAdapter(ArrayList<Object> dataset, RecyclerView recyclerView) {
		mDataset = dataset;
		attach(recyclerView);
		mGrid = false;
		mLoading = false;
	...
	public void append(@Nullable ProviderHeader group, ArrayList<MangaHeader> data) {
		if (data.isEmpty()) {
			return;
		}
	...
	public int getItemViewType(int position) {
		if (position == mDataset.size()) {
			...
			return mDataset.get(position) instanceof MangaHeader ? VIEW_ITEM : VIEW_HEADER;
		}
	}

	@Nullable
	public MangaHeader getItem(int pos) {
		Object obj = mDataset.get(pos);
		return obj instanceof MangaHeader ? (MangaHeader) obj : null;
	}
	...
	public final void onBindViewHolder(@NotNull RecyclerView.ViewHolder holder, int position) {
		if (holder instanceof MangaListAdapter.MangaViewHolder) {
			((MangaListAdapter.MangaViewHolder) holder).fill((MangaHeader) mDataset.get(position), mThumbSize, mChoiceController.isSelected(position));
			...
			((GroupViewHolder) holder).fill((ProviderHeader) mDataset.get(position));
		}
	}
	...
	private static class GroupViewHolder extends RecyclerView.ViewHolder {
		...
		public void fill(ProviderHeader data) {
            mTextView.setText(data.dName);
		}
	}
