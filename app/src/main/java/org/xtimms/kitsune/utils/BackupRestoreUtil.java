...
import org.xtimms.kitsune.core.helpers.ContentShareHelper;
...
public class BackupRestoreUtil {
    ...
    public static void showBackupDialog(final Context context) {
        String[] items = new String[]{
                ...
                context.getString(R.string.favourites_categories)
        };
    ...
    public static void showRestoreDialog(final Context context) {
        ...
        while (true) {
            if (i < files.length) {
                ...
                try {
                    items[i] = AppHelper.getReadableDateTime(Long.parseLong(str.substring(0, str.lastIndexOf('.'))));
            ...
            AlertDialog.Builder builder = (new AlertDialog.Builder(context))
                    ...
                    .setCancelable(true);
            if (items.length != 0) {
                builder.setItems((CharSequence[])items, (dialog, which) ->
                ...
                if (context.getPackageManager().resolveActivity(intent, 0) != null)
                    builder.setNeutralButton(R.string.import_file, new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            ((Activity)context).startActivityForResult(Intent.createChooser(intent, context.getString(R.string.import_file)),
                                    BACKUP_IMPORT_CODE);
                        }
                    });
            }
    ...
    public static File getExternalBackupDir() {
        File dir = new File(Environment.getExternalStorageDirectory(), "Kitsune");
        if (!dir.exists()) {
            dir.mkdirs();
        }
    ...
    public void backup(boolean[] what) {
        new BackupTask().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, what[0], what[1], what[2]);
    }
    ...
    private class BackupTask extends AsyncTask<Boolean, Integer, File> {
        ...
        protected File doInBackground(Boolean... params) {
            ...
            //backup categories
            if (params[2]) {
                publishProgress(R.string.favourites_categories);
                file = new File(dir, "categories.json");
                jsonArray = storageHelper.extractTableData("categories", null);
                if (jsonArray == null || writeToFile(file, jsonArray.toString())) {
                    errors++;
                }
            }
            ...
            try {
                ...
                zipBuilder.addFiles(Objects.requireNonNull(dir.listFiles())).build();
            } catch (IOException e) {
                file = null;
            } finally {
                if (zipBuilder != null) {
        ...
        protected void onPostExecute(final File file) {
            ...
            if (file != null) {
                new AlertDialog.Builder(mContext)
                        ...
                        .setPositiveButton(R.string.export_file, (dialog, which) -> new ContentShareHelper(mContext).exportFile(file))
                        .create().show();
            } else {
                new AlertDialog.Builder(mContext)
    ...
    private class RestoreTask extends AsyncTask<File, Integer, Boolean> {
        ...
        protected Boolean doInBackground(File... params) {
            ...
            try {
                FilesystemUtils.copyFile(params[0], file);
            } catch (IOException e) {
                return false;
            }
            ...
            if (file.exists()) {
                ...
                try {
                    data = new JSONArray(readFromFile(file));
                    if (storageHelper.insertTableData("categories", data)) {
                        errors++;
                    }
                } catch (Exception e) {
                    errors++;
            ...
            //restore bookmarks
            file = new File(dir, "bookmarks.json");
            if (file.exists()) {
                publishProgress(R.string.bookmarks);
                try {
                    data = new JSONArray(readFromFile(file));
                    if (storageHelper.insertTableData("bookmarks", data)) {
                        errors++;
                    }
                } catch (Exception e) {
                    errors++;
